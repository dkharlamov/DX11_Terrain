...
Landscape with seafloor texture
blue fog light
*/
...
ID3D11ShaderResourceView*           g_pTextureSky = NULL;
...
HRESULT InitDevice()
{
	...
	SimpleVertex *vertices = new SimpleVertex[512 * 512 * 6];

	for(int i = 0; i < 512; i++)
		for (int j = 0; j < 512; j++)
		{
			vertices[(i + 512 * j) * 6 + 0].Pos = XMFLOAT3(i,		 0,	 j);
			vertices[(i + 512 * j) * 6 + 1].Pos = XMFLOAT3(i,		 0,	 j + 1);
			vertices[(i + 512 * j) * 6 + 2].Pos = XMFLOAT3(i + 1,	 0,	 j + 1);
			vertices[(i + 512 * j) * 6 + 3].Pos = XMFLOAT3(i,		 0,	 j);
			vertices[(i + 512 * j) * 6 + 4].Pos = XMFLOAT3(i + 1,	 0,	 j + 1);
			vertices[(i + 512 * j) * 6 + 5].Pos = XMFLOAT3(i + 1,	 0,	 j);

			vertices[(i + 512 * j) * 6 + 0].Tex = XMFLOAT2((float)i / 512.0, (float)j / 512.0);
			vertices[(i + 512 * j) * 6 + 1].Tex = XMFLOAT2((float)i / 512.0, (float)(j + 1) / 512.0);
			vertices[(i + 512 * j) * 6 + 2].Tex = XMFLOAT2((float)(i + 1) / 512.0, (float)(j + 1) / 512.0);
			vertices[(i + 512 * j) * 6 + 3].Tex = XMFLOAT2((float)i / 512.0, (float)j / 512.0);
			vertices[(i + 512 * j) * 6 + 4].Tex = XMFLOAT2((float)(i + 1) / 512.0, (float)(j + 1) / 512.0);
			vertices[(i + 512 * j) * 6 + 5].Tex = XMFLOAT2((float)(i + 1) / 512.0, (float)j / 512.0);
		}


    ZeroMemory( &bd, sizeof(bd) );
    bd.Usage = D3D11_USAGE_DEFAULT;
	bd.ByteWidth = sizeof(SimpleVertex) * 512 * 512 * 6;
    bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bd.CPUAccessFlags = 0;
    ZeroMemory( &InitData, sizeof(InitData) );
    ...
    // Load the Texture
    hr = D3DX11CreateShaderResourceViewFromFile( g_pd3dDevice, L"mountains512.png", NULL, NULL, &g_pTextureRV, NULL );
    if( FAILED( hr ) )
        return hr;

	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"rock_huge.jpg", NULL, NULL, &g_pTextureRock, NULL);
	if (FAILED(hr))
		return hr;
...
UINT offset = 0;
	...
	T = XMMatrixTranslation(-130, -40, -130);

    // Update skybox constant buffer
    ConstantBuffer constantbuffer;
	constantbuffer.World	=	XMMatrixTranspose(worldmatrix * T);
	...
	g_pImmediateContext->PSSetShaderResources( 0, 1, &g_pTextureRock );
	g_pImmediateContext->VSSetShaderResources(0, 1, &g_pTextureRV);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer, &stride, &offset);
    g_pImmediateContext->PSSetSamplers( 0, 1, &g_pSamplerLinear );

    g_pImmediateContext->Draw( 512 * 512 * 6, 0 );
